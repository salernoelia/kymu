...
export class RenderStreaming {
  ...
  constructor(signaling, config) {
    ...
    this.onConnect = function (connectionId) {
      Logger.log(`Connect peer on ${connectionId}.`);
    };
    this.onDisconnect = function (connectionId) {
      Logger.log(`Disconnect peer on ${connectionId}.`);
    };
    this.onGotOffer = function (connectionId) {
      Logger.log(`On got Offer on ${connectionId}.`);
    };
    this.onGotAnswer = function (connectionId) {
      Logger.log(`On got Answer on ${connectionId}.`);
    };
    this.onTrackEvent = function (data) {
      Logger.log(`OnTrack event peer with data:${data}`);
    };
    this.onAddChannel = function (data) {
      Logger.log(`onAddChannel event peer with data:${data}`);
    };

    this._config = config;
    this._signaling = signaling;
    this._signaling.addEventListener("connect", this._onConnect.bind(this));
    this._signaling.addEventListener(
      "disconnect",
  ...
  async createConnection(connectionId) {
    ...
    console.log(`Create connection with id: ${this._connectionId}`);
  ...
  _preparePeerConnection(connectionId, polite) {
    ...
    this._peer.addEventListener("disconnect", () => {
      this.onDisconnect(
        `Receive disconnect message from peer. connectionId:${connectionId}`
      );
    });
