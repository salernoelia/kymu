...
// Inside therapybuilder.vue script setup
const handleModelLoaded = (model) => {
  const jointPathMap = initializeJointMap(model);

  // Debug: Log all accessible joints
  console.log("Available joints:", Object.keys(jointPathMap));

  // Debug: Try to find key joints to verify mapping
  ["Shoulder", "Elbow", "Neck"].forEach((region) => {
    console.log(
      `Testing ${region} joints:`,
      getSuggestedJointsForRegion(region as BodyRegion).map((joint) => {
        const found = findJointByName(joint);
        return { joint, found: !!found, path: found ? joint : "Not found" };
      })
    );
  });
};
