  ...
  <div>
    ...
    >
      ...
      <video
        ...
        playsInline
        @loadedmetadata="onLoadedVideo"
        @click="onClickPlayButton"
      />
    </div>
    <img
      id="fullscreenButton"
      ref="fullScreenButtonElement"
...
async function setupRenderStreaming() {
  ...
  renderstreaming.onTrackEvent = (data) => {
    if (
      videoPlayer !== null &&
      videoElement.value instanceof HTMLVideoElement
    ) {
...
function setCodecPreferences() {
  ...
  if (transceivers && transceivers.length > 0) {
    const validCodecs = selectedCodecs.filter(
      (codec): codec is RTCRtpCodec => codec !== undefined
    );
...
function showStatsMessage() {
  intervalId = setInterval(async () => {
    ...
    const array = createDisplayStringArray(
      stats as RTCStatsReport,
      lastStats as RTCStatsReport
    );
    if (array.length) {
      messageDisplay.value = "block";
